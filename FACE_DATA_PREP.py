...
##########################################DATASET PREPARATION##########################################
directory = "Face_Emotions"											#Destination directory which contains images.
emotions = ["Angry","Happy","Normal","Sad","Shocked"] 								#These folders will be accessed by the program for data.
...
for emotion in emotions: 											#First loop to select the folder.
	...
	for img in os.listdir(path): 										#Second loop to select the image.
		try:											#Some images might not load properly.
			img_array = cv2.imread(path+"/"+img,0)							#Load the image into the program as a numpy array.
...
np.save("Datasets/FACE_TRAIN.npy",training_data) 								#Save the training data in the Datasets folder.
#########################################################################################################
